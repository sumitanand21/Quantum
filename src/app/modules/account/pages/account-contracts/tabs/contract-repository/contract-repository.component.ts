...
import { ContractRequestAccesspopup } from '../pending-requests/pending-requests.component';
...
export class ContractRepositoryComponent implements OnInit {
  ContractRepositoryTable=[];
  ...
  ngOnInit(): void {
      var orginalArray = this.activerequest.getAllContractRepository();
      orginalArray.subscribe((x: any[]) => {
      this.ContractRepositoryTable = x;
...
performTableChildAction(childActionRecieved): Observable<any> {
  ...
  switch (actionRequired.action) {
    ...
    case 'Name':{
      ...
      this.accountlistService.setSession('routeParams', { 'route_from': '', 'Id':  actionRequired.objectRowData[0].id })
