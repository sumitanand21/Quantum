...
@Component({
  selector: 'app-all-active-accounts',
  templateUrl: './all-active-accounts.component.html',
  styleUrls: ['./all-active-accounts.component.scss']
})

export class AllActiveAccountsComponent implements OnInit, AfterViewInit {
  selectedTabId;
  tableTotalCount = 0;
  isLoading = false;
  tabList2;
  ...
  async ngOnInit() {
    localStorage.setItem('routeValue', this.EncrDecr.set('EncryptionEncryptionEncryptionEn', 10, 'DecryptionDecrip'))
    ...
    if (this.tabList2.length > 0) {
      const locally = this.tabList2[0].GroupData.filter(x => x.PinId === 10)[0];
      this.selectedTabId = locally.id;
      console.log('in oninit farming', this.selectedTabId);
    }
    this.isLoading = true;
    // this.store.pipe(select(farmingRequestState)).subscribe(
    //   (res: any) => {
    //     if (res) {
    //       if (res.ids.length > 0) {
    //         this.isLoading = false;
    //         this.AccountCreationActiveRequestsTable = this.getTableFilterData(Object.values(res.entities));
    //         this.AccountCreationActiveRequestsTable.map((resp, index) => {
    //           resp.index = index + 1;
    //         });
    ...
    //   console.log('cacheoffline', CacheResponse);
    //   if (CacheResponse) {
    //     if (CacheResponse.data.length > 0) {
    //       this.isLoading = false;
    //       this.AccountCreationActiveRequestsTable = this.getTableFilterData(CacheResponse.data);
    //       this.tableTotalCount = CacheResponse.count;
    //       this.ActiveAccountRequest.OdatanextLink = CacheResponse.OdatanextLink;
    //     }
    //   }
    // }
    this.GetAllHistory(this.ActiveAccountRequest, true);
    ...
    console.log('in farming', this.tabList2);
    if (this.tabList2.length > 0) {
      const locally = this.tabList2[0]['GroupData'].filter(x => x.PinId === 10);
      this.selectedTabId = this.tabList2[0].GroupData.filter(x => x.PinId === 10)[0].id;
    ...
    switch (actionRequired.action) {
      case 'Name': {
        const accountcontacts = {
          ...
          'Classification': actionRequired.objectRowData[0].Classification,
  ...
  downloadList(data): void {
    ...
    this.accountListService.getFilterList(reqparam, true, 'MyActiveAccountsList').subscribe(res => {
      if (!res.IsError) {
        this.isLoading = false;
  ...
  CallListDataWithFilters(data,isConcat?) {
    const reqparam = this.GetAppliedFilterData({ ...data }, true);
    this.accountListService.getFilterList(reqparam, false, 'FilterMyActiveAccounts').subscribe(res => {
      if (!res.IsError) {
        if (res.ResponseObject.length > 0) {
  ...
  generateFilterConfigData(data, headerName, isConcat, isServiceCall?) {
    if (isServiceCall) {
      ...
      this.accountListService.getFilterSwitchListData({ columnFIlterJson: this.GetAppliedFilterData(data, false), useFulldata: useFulldata }, 'SearchFilterAPIChange', 'myactiveAccount')
        .subscribe(res => {
  ...
  CustomDropdown(data) {
    ...
    if (data.objectRowData.PinId === 10) {
      return;
    ...
    } else if (data.objectRowData.PinId === 11) {
  ...
  GetAllHistory(reqBody, isConcat): void {
    this.isLoading = true;
    this.accountListService.getAllActiveAccount(reqBody)
