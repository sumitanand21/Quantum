...
export class AllianceComponent implements OnInit {
  ...
  // api call to here
  async ngOnInit() {
    localStorage.setItem('routeValue', this.EncrDecr.set('EncryptionEncryptionEncryptionEn', 12, 'DecryptionDecrip'))
    ...
    if (this.tabList2.length > 0) {
      const local = this.tabList2[0].GroupData.filter(x => x.PinId === 12)[0];
      this.selectedTabId = local.id;
      console.log('in oninit farming', this.selectedTabId);
    }
    // this.store.pipe(select(AllianceRequestState)).subscribe(res => {
    //   if (res) {
    //     if (res.ids.length > 0) {
    //       this.isLoading = false;
    //       this.AccountCreationAllianceTable = this.getTableFilterData(Object.values(res.entities));
  ...
  //       this.AccountAllianceServ.accountSearch(searchData, 2, this.allianceReqBody.PageSize, this.allianceReqBody.OdatanextLink, this.allianceReqBody.RequestedPageNumber).subscribe(res => {
  //         if (!res.IsError) {
  ...
  performTableChildAction(childActionRecieved): Observable<any> {
    ...
    switch (actionRequired.action) {
      ...
      case 'view modification': {
        this.AccountAllianceServ.setSession('routeParams', { 'route_from': 'acc_req', 'Id': actionRequired.objectRowData[0].id });
  ...
  downloadList(data): void {
    ...
    this.AccountAllianceServ.getFilterList(reqparam, true, 'AllianceAccountsList').subscribe(res => {
      if (!res.IsError) {
        this.isLoading = false;
  ...
  CallListDataWithFilters(data, isConcat?) {
    const reqparam = this.GetAppliedFilterData({ ...data }, true);
    this.AccountAllianceServ.getFilterList(reqparam, false, 'FilterAllianceAccounts').subscribe(res => {
      if (!res.IsError) {
        if (res.ResponseObject.length > 0) {
  ...
  generateFilterConfigData(data, headerName, isConcat, isServiceCall?) {
    if (isServiceCall) {
      ...
      // tslint:disable-next-line:max-line-length
      this.AccountAllianceServ.getFilterSwitchListData({ columnFIlterJson: this.GetAppliedFilterData(data, false), useFulldata: useFulldata }, 'SearchFilterAPIChange', 'alliance').subscribe(res => {
  ...
  GetAllHistory(reqBody, isConcat): void {
    this.isLoading = true;
    this.AccountAllianceServ.getAllAlliance(reqBody)
      .subscribe(async (activeRequest) => {
        if (!activeRequest.IsError) {
          ...
          if (activeRequest.ResponseObject.length > 0) {
            ...
            // tslint:disable-next-line:max-line-length
            this.store.dispatch(new AllianceAccountAction({ AllianceListModel: ImmutableObject.ResponseObject, count: activeRequest.TotalRecordCount, OdatanextLink: activeRequest.OdatanextLink }));
    ...
    if (tableData) {
      if (tableData.length > 0) {
        return tableData.map((alliance => {
          return {
            id: alliance.SysGuid,
            Name: this.AccountAllianceServ.getSymbol(alliance.Name) || '',
            ...
            index: alliance.index ? alliance.index : '',
