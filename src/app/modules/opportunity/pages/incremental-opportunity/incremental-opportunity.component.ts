...
@Component({
  selector: 'app-incremental-opportunity',
  templateUrl: './incremental-opportunity.component.html',
  styleUrls: ['./incremental-opportunity.component.scss']
})
export class IncrementalOpportunityComponent implements OnInit {
...
//     this.projectService.getAccountName(data).subscribe((res) => {

//       if (!res.error) {
//          if(res.ResponseObject != null && res.ResponseObject.length>0){

//             this.accountNameArray = res.ResponseObject;
...
//   }
  ...
  search(paginationPageNo) {
    ...
    console.log(this.oppNameNo);

    if (this.accountId) {
      this.isLoading= true;

    this.projectService.GetBFMApprovedOrderList(this.oppNameNo, this.orderNumber, this.accountId,
     paginationPageNo.RequestedPageNumber,paginationPageNo.PageSize,  this.searchOppo  
    ).subscribe((res) => {
    ...
    this.tableTotalCount = res.TotalRecordCount;
       this.isLoading= false;
        ...
        console.log(this.responseObjectArr);
    ...
    var tableCollection=  this.responseObjectArr.map((item, index) => {
               ...
               return {
                ...
                "Opportunity": item.OpportunityId?item.OpportunityId:''
          }
...
OpportunityId=''
   createpop() {
    ...
    this.projectService.getRenewal( this.order ,this.primOrderId, this.primOppId, numInc,this.userGuid,OwnerName,OwnerEmailId).subscribe((res) => {
      ...
      if (this.oppNum !="" && this.oppNum != null) {
        this.isLoading=false;
      const dialogRef = this.dialog.open(createpopupcomponent2,
        {
          width: '350px',
...
this.selectedAccNameObj = { SysGuid: "(sessionStoragesessionStorage.getItem('accountSysId')) ? this.EncrDecr.get('EncryptionEncryptionEncryptionEn', sessionStorage.getItem('accountSysId'), 'DecryptionDecrip') : ''" , 
...
generateFilterConfigData(data, headerName,isConcat, isServiceCall?) {
 if(data.filterData.headerName=='enddate' || data.filterData.headerName=='Startdate'){
    ...
    if (isServiceCall) {
      ...
      this.projectService.getrenewalListConfigData({ ...data,useFullData:useFulldata, searchOppo:this.searchOppo?this.searchOppo:'', columnFIlterJson: this.GetAppliedRenewalFilterData(data), isService:true , "OrderNumber": this.orderNumber?this.orderNumber:'',"OrderStatus": "","OpportunityNumber": this.oppNameNo?this.oppNameNo:'',
         "OpportunityName": this.oppNameNo?this.oppNameNo:'',"AccountId": this.accountId?this.accountId:'',
...
export class createpopupcomponent2 {
  constructor(public router: Router, public dialogRef: MatDialogRef<createpopupcomponent2>, public dialog: MatDialog,
