import { Injectable } from '@angular/core';
import { Actions, Effect, ofType } from '@ngrx/effects';
import { OfflineService } from '@app/core/services/offline.services';
import { tap } from 'rxjs/operators';
import { DealsActionTypes, TaggedListAction } from '../../actions/deals.actions';
import { dealService } from '@app/core/services/deals.service';
@Injectable()
export class TaggedDealsEffects {

    @Effect({ dispatch: false })
    TaggedDeals$ = this.actions$.pipe(
        ofType<TaggedListAction>(DealsActionTypes.TaggedListAction),
        tap(async action => {
            console.log("effets action display")
            console.log(action)
            await this.offlineService.ClearExistingDealsData()
            this.offlineService.addTaggedDealsCacheData(this.dealService.DealCacheType.Table, action.payload.taggedDealslist)
        })
    )


    constructor(private actions$: Actions, private offlineService: OfflineService, private dealService: dealService) { }

}